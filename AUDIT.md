...
- [x] **`pkg/integration/guild_vehicle` cross-package integrations missing** — `pkg/integration/guild_vehicle/doc.go:60-69` — the package self-documents four planned integrations as "FUTURE (not yet implemented)": federation guild membership/permissions, `VehicleComponent`/`VehicleCombatComponent` sync, formation-based physics behavior, and siege damage application to territory. The standalone fleet-management API (creation, formations, persistence, access control) is fully implemented, but no other system in the project consumes it. **Blocked goal:** README guild systems × vehicle systems integration. **Remediation completed:** (1) `MembershipValidator` interface added; `guild.Manager.IsMember` implemented; wired in `cmd/server/main.go` so `FleetManager.GrantAccess` enforces guild membership server-side. (2) `StructureDamager` interface + `ApplySiegeDamage` added; wired to `territory.Manager` in `cmd/server/main.go` for server-authoritative siege damage. (3) `GetFormationOffsets` implemented for all four formation types for `pkg/engine/physics/vehicle` integration. (4) `VehicleSyncer` interface added for ECS engine integration; not yet wired at a construction site because `pkg/engine` already imports `guild_vehicle` (import cycle constraint); hook is available for any engine-layer type that implements `VehicleSyncer`. Validate: `go test ./pkg/integration/guild_vehicle/... ./pkg/network/federation/guild/...`.
