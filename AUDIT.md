...
- [x] **Respec lacks gold check — players can respec for free** — `cmd/client/handlers.go:3260` — `TODO: Check if player has enough gold for respec`. The prestige UI `SetRespecCallback` always returns `true` regardless of player gold. This bypasses the intended economic cost of skill respecs. — **Remediation:** Retrieve player's `GoldComponent`, compare against `cost`, return `false` if insufficient; display feedback message. — **FIXED:** Implemented gold check in respec callback using InventoryComponent.Gold field.
