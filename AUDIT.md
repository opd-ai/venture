...
- [ ] `LightingConfig.EnableShadows` is defined but not consumed by rendering logic — `/home/runner/work/venture/venture/pkg/rendering/lighting/types.go:116-122` explicitly documents no-op status; usage scan shows only type/default/test references in lighting package — partially wired config path creates misleading API surface — **Remediation:** either (a) wire `EnableShadows` into lighting render/update pipeline with concrete behavior and tests, or (b) remove public field and route shadow toggling through already active engine shadow controls with migration note.
