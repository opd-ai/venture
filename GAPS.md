...
- **Current State**: `cmd/client/init_versions.go:644-645` calls `sys.tradeRouteManager.Start()`, which launches a background goroutine (protected by `sync.Once`) with a `10 * time.Second` `time.Ticker`. The client's cleanup function at `cmd/client/handlers.go:3576-3602` calls `cleanupNetworkClient()` and the embedded server's cleanup, but has no call to `sys.tradeRouteManager.Stop()`. The goroutine and ticker run until the OS reclaims the process (acceptable for production exit) but cause false positives in `-race` testing and complicate graceful shutdown testing.
- **Blocked Goal**: No user-visible goal is blocked; this is a resource management correctness issue.
- **Implementation Path**: Add `defer sys.tradeRouteManager.Stop()` in `initializeV19SystemsClient()` at `cmd/client/init_versions.go` immediately after the `sys.tradeRouteManager.Start()` call at line 645, or register the stop call in the cleanup closure returned by the relevant initialisation step. `Stop()` is idempotent (uses `sync.Once`). Validate with `go test -race ./pkg/integration/trade_routes/`.
- **Dependencies**: None.
