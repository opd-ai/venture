...
- [ ] CraftingSystem created with nil itemGen on mobile path - Location: `pkg/engine/system_init.go:1168` - Impact: `InitializeGameSystems()` (used by `cmd/mobile/mobile.go:164`) creates the CraftingSystem with `nil` for itemGen: `NewCraftingSystem(game.World, inventorySystem, nil)`. Comment says "itemGen set later" but no subsequent code on the mobile path ever sets it. The desktop client (`cmd/client/handlers.go:988`) correctly creates it with `sys.itemGen`. This means crafting on mobile will fail/crash when attempting to generate crafted items. Dependency: Requires passing an `item.ItemGenerator` to `InitializeGameSystems` config or setting it post-init in `cmd/mobile/mobile.go`.

## High Priority (Degrades Experience)

### Division by Zero Risk in Terrain Renderer
- [ ] No validation that tileWidth/tileHeight are non-zero before division - Location: `pkg/engine/terrain_render_system.go:109-112` - Impact: `NewTerrainRenderSystem()` (line 36) accepts `tileWidth, tileHeight int` without validating they are positive. Lines 109-112 divide viewport coordinates by `float64(t.tileWidth)` and `float64(t.tileHeight)`. If either is 0 (e.g., from misconfiguration or default zero-value struct), this causes a float64 division producing +Inf, leading to infinite tile iteration loops that freeze the game. Fix: add `if t.tileWidth <= 0 || t.tileHeight <= 0 { return }` guard at start of `Draw()`.
...
- [ ] Direct array append instead of using `InventorySystem.AddItemToInventory()` - Location: `pkg/engine/item_spawning.go:714-722` (approx.) - Impact: `ItemPickupSystem.attemptItemPickup()` directly appends to `inventory.Items` instead of calling `InventorySystem.AddItemToInventory()` (defined at `pkg/engine/inventory_system.go:54`). This bypasses: (1) inventory UI update notifications, (2) item acquisition event emissions, (3) any validation or logging in the inventory system. Result: inventory UI may not update when items are picked up, and item acquisition events are not fired for achievement/quest tracking.
