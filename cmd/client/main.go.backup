...
func main() {
	...
	// Calculate player spawn position and create player entity
	playerX, playerY := calculatePlayerSpawnPosition(generatedTerrain, clientLogger)
	player := createPlayerEntity(game, playerX, playerY, animationSystem, clientLogger)

	// Store player entity reference for death callback
	playerEntity = player

	// Initialize and wire up commerce UI
	...
	// Add gameplay components to player
	_, _ = addPlayerComponents(player, logger, clientLogger)
