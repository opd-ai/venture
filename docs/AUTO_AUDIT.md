# TASK DESCRIPTION:
Autonomously analyze a mature Go application to identify implementation gaps across its codebase, documentation, and observed behavior, then automatically implement production-ready solutions to resolve all-priority issues. The tutorial starts with "Press space to continue" but pressing the space bar does nothing. Ensure that all controls are correctly wired up to the game.
