...
## OBJECTIVE
Identify and implement the next incomplete Phase from docs/PLAN.md (short-term) or docs/ROADMAP_V4.md (long-term). Prioritize PLAN.md Phases. If PLAN.md is complete, verify thoroughly then delete it. Complete exactly ONE Phase following Venture's ECS architecture, procedural generation patterns, and testing standards. Work in complete sections, implement the entire objective defined by the planning document, do not stop until you are finished. You are not done until all tests pass. We have multiple `ROADMAP_V*.md` files in the codebase, choose the first unfinished item from the lowest-numbered unfinished roadmap to complete. Look for empty `[ ]` checkboxes before continuing on to later roadmap file. ROADMAP_V5.md is currently *INCOMPLETE*, if you exceed it you are wrong. When ROADMAP_V5.md is finally complete, remove this notice. There are some ROADMAP_V5.md items that are complete but inadequately documented in the ROADMAP_V5.md file. If you encounter this, update the ROADMAP_V5.md and consider the task complete. Verify ZERO empty checkboxes before moving on. THERE should be zero empty checkboxes before moving on to ROADMAP_V6.md.
