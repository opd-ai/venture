...
3. **Audit and migrate unstructured logging to Logrus**
   ...
   - Files to modify (current known violations):
     ...
     - `pkg/engine/mail_doc.go` (contains a `fmt.Printf` only inside a documentation comment/example; not runtime production code, excluded from migration scope).
   - Approach:
   ...
   - Acceptance criteria: `grep -rn 'fmt\.Print\|log\.Print' pkg/ cmd/ --include='*.go' | grep -v _test.go | grep -v doc.go | grep -v examples/ | grep -Ev '^\s*//'` returns zero matches (excluding in-comment references).
