...
### Low Severity
- [ ] **Table-driven test opportunity** — Test functions in `balance_test.go` and `validators_test.go` follow similar structure but are not table-driven. Consider parameterized test for all 8 validators with expected metric keys. (`balance_test.go:9-102`, `validators_test.go:9-200`)
