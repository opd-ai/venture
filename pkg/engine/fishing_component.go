...
// AddFishType adds a fish type to this spot's population.
func (f *FishingSpotComponent) AddFishType(fishTypeID string, spawnWeight float64) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.FishPopulation[fishTypeID] = spawnWeight
}

// RemoveFishType removes a fish type from this spot.
func (f *FishingSpotComponent) RemoveFishType(fishTypeID string) {
	f.mu.Lock()
	defer f.mu.Unlock()
	delete(f.FishPopulation, fishTypeID)
}

// GetFishTypes returns all fish type IDs at this spot.
func (f *FishingSpotComponent) GetFishTypes() []string {
	f.mu.RLock()
	defer f.mu.RUnlock()
	types := make([]string, 0, len(f.FishPopulation))
	for id := range f.FishPopulation {
		types = append(types, id)
	}
	return types
}

// GetSpawnWeight returns the spawn weight for a fish type.
func (f *FishingSpotComponent) GetSpawnWeight(fishTypeID string) float64 {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.FishPopulation[fishTypeID]
}

// CanFish checks if another fisher can use this spot.
func (f *FishingSpotComponent) CanFish() bool {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.IsActive && f.CurrentFishers < f.MaxConcurrentFishers && f.CooldownTimer <= 0
}

// AddFisher increments the current fisher count.
func (f *FishingSpotComponent) AddFisher() bool {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.CurrentFishers >= f.MaxConcurrentFishers {
		return false
	}
	f.CurrentFishers++
	return true
...
// RemoveFisher decrements the current fisher count.
func (f *FishingSpotComponent) RemoveFisher() {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.CurrentFishers > 0 {
		f.CurrentFishers--
	}
}

// GetCurrentFishers returns the current fisher count.
func (f *FishingSpotComponent) GetCurrentFishers() int {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.CurrentFishers
}

// UpdateCooldown processes cooldown timer.
func (f *FishingSpotComponent) UpdateCooldown(deltaTime float64) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.CooldownTimer > 0 {
		f.CooldownTimer -= deltaTime
		if f.CooldownTimer < 0 {
			f.CooldownTimer = 0
		}
	}
...
// SetCooldown sets the cooldown timer.
func (f *FishingSpotComponent) SetCooldown(seconds float64) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.CooldownTimer = seconds
}
