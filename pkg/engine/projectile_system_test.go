...
func TestProjectileSystem_SetQuadtree(t *testing.T) {
	...
	qt := NewQuadtree(0, 0, 1000, 1000, 4)
	sys.SetQuadtree(qt)
...
func TestProjectileSystem_SpawnProjectile(t *testing.T) {
	...
	if posComp.X != 100.0 || posComp.Y != 200.0 {
	...
	if velComp.VX != 300.0 || velComp.VY != 0.0 {
		t.Errorf("Velocity = (%v, %v), want (300, 0)", velComp.VX, velComp.VY)
	}

	// Check projectile component
	proj, ok := entity.GetComponent("projectile").(*ProjectileComponent)
	if !ok {
...
func TestProjectileSystem_Update_Movement(t *testing.T) {
	...
	posComp, _ := entity.GetComponent("position").(*PositionComponent)
	expectedX := 100.0 + 400.0*1.0 // old + velocity * time
	if posComp.X != expectedX {
		t.Errorf("Position X = %v, want %v", posComp.X, expectedX)
	}
	if posComp.Y != 100.0 {
		t.Errorf("Position Y = %v, want 100", posComp.Y)
	}

	// Check age increased
	proj, _ := entity.GetComponent("projectile").(*ProjectileComponent)
	if proj.Age != 1.0 {
...
func TestProjectileSystem_EntityCollision(t *testing.T) {
	...
	target.AddComponent(&HealthComponent{CurrentHealth: 100.0, MaxHealth: 100.0})
	...
	sys.Update(0.5) // Move projectile to ~300, should hit target at 200
	...
	health, _ := target.GetComponent("health").(*HealthComponent)
	expectedHealth := 100.0 - 25.0
	if health.CurrentHealth != expectedHealth {
		t.Errorf("Target health = %v, want %v", health.CurrentHealth, expectedHealth)
	}
}
...
func TestProjectileSystem_PierceCollision(t *testing.T) {
	...
	target1.AddComponent(&HealthComponent{CurrentHealth: 100.0, MaxHealth: 100.0})

	target2 := w.CreateEntity()
	target2.AddComponent(&PositionComponent{X: 160.0, Y: 100.0})
	target2.AddComponent(&HealthComponent{CurrentHealth: 100.0, MaxHealth: 100.0})
	...
	sys.Update(0.15) // Move to ~130, hit target1
	...
	health1, _ := target1.GetComponent("health").(*HealthComponent)
	if health1.CurrentHealth >= 100.0 {
		t.Error("First target should have taken damage")
	}

	// Update again - should hit second target and despawn
	sys.Update(0.25) // Move to ~180, hit target2
	...
	health2, _ := target2.GetComponent("health").(*HealthComponent)
	if health2.CurrentHealth >= 100.0 {
...
func TestProjectileSystem_ExplosiveProjectile(t *testing.T) {
	...
	target3.AddComponent(&HealthComponent{CurrentHealth: 100.0, MaxHealth: 100.0})
	...
	damage1 := 100.0 - health1.CurrentHealth
	damage2 := 100.0 - health2.CurrentHealth
	if damage1 <= damage2 {
		t.Error("Closer target should take more explosion damage")
	}

	// Target 3 should not take damage (outside radius)
	health3, _ := target3.GetComponent("health").(*HealthComponent)
	if health3.CurrentHealth != 100.0 {
