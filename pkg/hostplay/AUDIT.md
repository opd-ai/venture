...
- [x] **Time Dependency** — `time.Now()` used in production code (`time_provider.go:19`). While properly abstracted via TimeProvider interface for testing, this creates non-deterministic behavior in production. Consider using an injected game clock for full determinism (see `pkg/engine/game_clock.go`) — **ACCEPTABLE**: `RealTimeProvider.Now()` is an intentional wrapper; the TimeProvider interface already provides full test isolation. Hostplay server session timing (connection, shutdown) is real-time management, not procedural generation that requires seed determinism. The godoc already explains the design ("enables deterministic testing by injecting mock time providers"). No change needed.
