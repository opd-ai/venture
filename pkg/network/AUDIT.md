...
### High Severity
- [x] **Non-deterministic timing** — **ACCEPTABLE**: networking timing (ping/timeout/bandwidth) is real-time by definition; time.Now() in network stack is correct. Guideline #2 applies to procedural generation, not network monitoring.
...
- [x] **No GameClock injection** — **DEFERRED**: injecting GameClock into TCPClient/TCPServer/ChatManager/BandwidthMonitor is a large refactoring; network time-based behavior is fundamentally real-time, making clock injection of limited test value.
