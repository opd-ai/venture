...
### Medium Severity
- [x] **Test Execution** — X11 test dependency — **ACCEPTABLE**: engine import requires Ebiten/GLFW; 30% coverage target exception applies (Ebiten-dependent package). Consistent with other rendering-adjacent packages.
...
- [x] **Template Hardcoding** — Template hardcoding — **ACCEPTABLE**: procgen standards use seed-based code-defined templates; mod system uses JSON rules but recipe templates are core content, intentionally code-defined.
...
- [x] **Material Quantity Range** — **DEFERRED**: adding quantity range to RecipeTemplate is a data model change affecting all template definitions; schedule for recipe system v2.
