...
### Low Severity
- [ ] **Settings Menu Integration** — Display resolution picker not exposed in settings menu UI. Settings persistence exists (`pkg/engine/settings.go` has `WindowWidth`, `WindowHeight`, `Fullscreen` fields), but no menu UI allows runtime resolution changes from in-game settings screen. F11 fullscreen toggle works, but resolution dropdown missing. (`cmd/client/init_versions.go:362-397`)
