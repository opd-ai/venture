...
func (m *MailboxUI) LoadFromMailComponent(mailComp *engine.MailComponent) {
	// Clear existing data
...
func (m *MailboxUI) isMessageUnread(msg *engine.MailMessage) bool {
...
func (m *MailboxUI) getStatusColor(status engine.MailStatus) color.Color {
	switch status {
	case engine.MailStatusDelivered:
		return m.DeliveredColor
	case engine.MailStatusInTransit:
		return m.InTransitColor
	case engine.MailStatusFailed:
...
func (m *MailboxUI) lightenColor(col color.Color, amount float64) color.Color {
	...
	factor := 1.0 + amount
	return color.RGBA{
		R: uint8(min(255.0, float64(r>>8)*factor)),
		G: uint8(min(255.0, float64(g>>8)*factor)),
		B: uint8(min(255.0, float64(b>>8)*factor)),
		A: uint8(a >> 8),
	}
}
