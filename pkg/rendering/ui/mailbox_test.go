...
func TestLoadFromMailComponent(t *testing.T) {
	...
	if ui.OutboxMessages[0].Status != engine.MailStatusInTransit {
...
func TestRenderEmptyMailbox(t *testing.T) {
	ui := NewMailboxUI(0, 0, 600, 400, "fantasy")
	mailComp := engine.NewMailComponent() // Empty mailbox
...
func TestGetStatusColor(t *testing.T) {
	...
	tests := []struct {
		...
		{engine.MailStatusDelivered, "should return DeliveredColor"},
		{engine.MailStatusInTransit, "should return InTransitColor"},
		{engine.MailStatusFailed, "should return FailedColor"},
		{engine.MailStatusSent, "should return TextColor"},
	}
...
func BenchmarkLoadFromMailComponent(b *testing.B) {
	...
	now := time.Now().Unix()
	for i := 0; i < 50; i++ {
		mailComp.Inbox = append(mailComp.Inbox, &engine.MailMessage{
