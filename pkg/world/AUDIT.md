...
- [ ] **Unstructured logging in examples** — README.md and doc.go contain example code using `log.Fatal`, `log.Fatalf`, `fmt.Printf` instead of structured `logrus.WithFields` logging. Examples should demonstrate best practices. (`economy/README.md:71,85,89,95,100,112,118,124,130,136,138`, `economy/doc.go:64,76,82,87,93,104,110,116`)
...
### Low Severity
- [ ] **Missing benchmarks** — No benchmarks found for hot-path code: chunk compression/decompression, persistence save/load, territory control point updates, pricing engine calculations. Performance-critical code should include benchmarks to prevent regressions.
