...
func TestDocumentedFilterOptions(t *testing.T) {
	...
	// Add test blueprints
	bp1 := NewBlueprint("Manor 1", "Player1", "fantasy", &BuildingDefinition{Width: 24, Height: 24})
	bp1.Tags = []string{"medieval", "manor"}
	bp1.AddRating(4.5)
	library.Add(bp1)

	bp2 := NewBlueprint("Manor 2", "Player2", "fantasy", &BuildingDefinition{Width: 32, Height: 32})
	bp2.Tags = []string{"medieval", "manor", "large"}
	bp2.AddRating(3.5)
	library.Add(bp2)

	bp3 := NewBlueprint("Tower 1", "Player1", "fantasy", &BuildingDefinition{Width: 16, Height: 16})
