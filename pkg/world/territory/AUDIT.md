...
### High Severity
- [ ] **Server Integration Gap** — TerritorySystem and TerritorySiegeSystem are registered in the **client only** (`cmd/client/handlers.go:486-557`, `cmd/client/init_versions.go:633-649`) but **missing from the server**. No references in `cmd/server/v9_systems.go` (83 lines) or `cmd/server/main.go` (system initialization at lines 200-600). This violates the authoritative server architecture: territory capture, war declarations, and siege progression run client-side, enabling exploits (fake capture progress, war cost bypass) and desynchronization in multiplayer. (`cmd/server/v9_systems.go:1-83`, `cmd/server/main.go:200-600`)
- [ ] **No Persistence Layer** — Manager and Siege structs lack `Serialize()`/`Deserialize()` methods. Territory ownership, war declarations (cost: 1000 gold, duration: 7 days), defensive structures (HP: 500-5000, damage: 0-200), and siege state (participants, capture progress, loot) will be **lost on server restart**. No references to `ComponentSerializer` interface or `pkg/saveload/` integration. (`manager.go:1-499`, `siege.go:1-525`, types.go:1-133`)
- [ ] **No Network Sync** — Territory and Siege structs are not encoded in network snapshots. No references in `pkg/network/snapshot.go` or replication code. Territory ownership changes, capture progress updates (updated every 1-60 seconds in `UpdateCaptureProgress`), war declarations, and siege phase transitions (Preparation → Assault → Resolution → Ended) will **not replicate to clients**, causing multiplayer desync. Server holds authoritative state but clients never receive updates. (`manager.go:108-142`, `siege.go:217-258`, `pkg/network/snapshot.go:1-500`)
